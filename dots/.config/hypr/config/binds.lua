...
hl.bind("SUPER + SHIFT + RETURN", hl.dsp.exec_cmd("uwsm app -- kitty claude --dangerously-skip-permissions"))
...
hl.bind("SUPER + SHIFT + V", hl.dsp.exec_cmd("~/.local/bin/desktop-vnc-toggle"), { description = "Toggle desktop VNC (LAN)" })
...
-- ─── Scrolling Layout ──────────────────────────
-- Only act when the active layout is "scrolling" (inert in dwindle/master).
-- Plain SUPER + arrows already move focus between columns.
hl.bind("SUPER + ALT + RIGHT", hl.dsp.layout("move +col"),  { description = "Scroll: move window to next column" })
hl.bind("SUPER + ALT + LEFT",  hl.dsp.layout("move -col"),  { description = "Scroll: move window to prev column" })
hl.bind("SUPER + ALT + SHIFT + RIGHT", hl.dsp.layout("swapcol r"), { description = "Scroll: swap column right" })
hl.bind("SUPER + ALT + SHIFT + LEFT",  hl.dsp.layout("swapcol l"), { description = "Scroll: swap column left" })
hl.bind("SUPER + ALT + I", hl.dsp.layout("consume"),        { description = "Scroll: consume next window into column" })
hl.bind("SUPER + ALT + O", hl.dsp.layout("expel"),          { description = "Scroll: expel window to its own column" })
hl.bind("SUPER + ALT + F", hl.dsp.layout("fit active"),     { description = "Scroll: fit active column to view" })
hl.bind("SUPER + ALT + C", hl.dsp.layout("center"),         { description = "Scroll: center active column" })
hl.bind("SUPER + ALT + W", hl.dsp.layout("colresize +conf"),{ description = "Scroll: cycle column width" })
...
-- ─── Workspaces 21–30 (SUPER+CTRL focus, +SHIFT move) ──
hl.bind("SUPER + CTRL + 1", hl.dsp.focus({ workspace = 21 }))
hl.bind("SUPER + CTRL + SHIFT + 1", hl.dsp.window.move({ workspace = 21 }))
hl.bind("SUPER + CTRL + 2", hl.dsp.focus({ workspace = 22 }))
hl.bind("SUPER + CTRL + SHIFT + 2", hl.dsp.window.move({ workspace = 22 }))
hl.bind("SUPER + CTRL + 3", hl.dsp.focus({ workspace = 23 }))
hl.bind("SUPER + CTRL + SHIFT + 3", hl.dsp.window.move({ workspace = 23 }))
hl.bind("SUPER + CTRL + 4", hl.dsp.focus({ workspace = 24 }))
hl.bind("SUPER + CTRL + SHIFT + 4", hl.dsp.window.move({ workspace = 24 }))
hl.bind("SUPER + CTRL + 5", hl.dsp.focus({ workspace = 25 }))
hl.bind("SUPER + CTRL + SHIFT + 5", hl.dsp.window.move({ workspace = 25 }))
hl.bind("SUPER + CTRL + 6", hl.dsp.focus({ workspace = 26 }))
hl.bind("SUPER + CTRL + SHIFT + 6", hl.dsp.window.move({ workspace = 26 }))
hl.bind("SUPER + CTRL + 7", hl.dsp.focus({ workspace = 27 }))
hl.bind("SUPER + CTRL + SHIFT + 7", hl.dsp.window.move({ workspace = 27 }))
hl.bind("SUPER + CTRL + 8", hl.dsp.focus({ workspace = 28 }))
hl.bind("SUPER + CTRL + SHIFT + 8", hl.dsp.window.move({ workspace = 28 }))
hl.bind("SUPER + CTRL + 9", hl.dsp.focus({ workspace = 29 }))
hl.bind("SUPER + CTRL + SHIFT + 9", hl.dsp.window.move({ workspace = 29 }))
hl.bind("SUPER + CTRL + 0", hl.dsp.focus({ workspace = 30 }))
hl.bind("SUPER + CTRL + SHIFT + 0", hl.dsp.window.move({ workspace = 30 }))
...
hl.bind("SUPER + SHIFT + R", hl.dsp.exec_cmd("qs -c noctalia-shell ipc call plugin:screen-toolkit record"))
...
hl.bind("CTRL + SUPER + ALT + SLASH", hl.dsp.exec_cmd("xdg-open ~/.config/hypr/config/binds.lua"))
...
-- XF86AudioPlay freed for voice-line's dictate.py hotkey (2026-07-21) - confirmed
-- via live evdev capture that this keyboard's Play/Pause button only ever sends
-- KEY_PLAYPAUSE, never KEY_PAUSE, so the XF86AudioPause bind below was already
-- dead weight from this hardware and nothing else competes for this key now.
hl.bind("XF86AudioPause", hl.dsp.exec_cmd("playerctl play-pause"), { locked = true })
hl.bind("XF86AudioStop", hl.dsp.exec_cmd("playerctl stop"), { locked = true })
hl.bind("CTRL + SUPER + RIGHT", hl.dsp.exec_cmd("playerctl next"), { description = "Media: next track" })
hl.bind("CTRL + SUPER + LEFT", hl.dsp.exec_cmd("playerctl previous"), { description = "Media: previous track" })

hl.bind(
    "Control_R",
    hl.dsp.exec_cmd("ydotool click 0xC0")
)
