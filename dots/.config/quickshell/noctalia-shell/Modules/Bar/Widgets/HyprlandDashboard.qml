...
Item {
  ...
  NIconButton {
    id: button
    icon: customIcon || "terminal"
    tooltipText: {
      if (!screen || PanelService.getPanel("hyprlandDashboardPanel", screen)?.isPanelOpen) return "";
      return "Hyprland IPC";
    }
    ...
    onClicked: {
      var panel = PanelService.getPanel("hyprlandDashboardPanel", screen);
      panel?.toggle(this);
    }
    onRightClicked: {
