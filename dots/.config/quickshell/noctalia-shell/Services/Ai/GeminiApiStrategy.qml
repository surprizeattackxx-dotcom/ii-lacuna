...
ApiStrategy {
    ...
    // Inlined helpers (replaces ii's CF.FileUtils / CF.StringUtils)
    function trimFileProtocol(path) { return ("" + path).replace(/^file:\/\//, ""); }
    function shellSingleQuoteEscape(str) { return ("" + str).replace(/'/g, "'\\''"); }

    function buildEndpoint(model): string {
        return model.endpoint + `?key=\$\{${apiKeyEnvVarName}\}`;
    }
    ...
    function parseBuffer(message) {
        ...
        try {
            ...
            const annotationSources = dataJson.candidates[0]?.groundingMetadata?.groundingChunks?.map(chunk => {
                return { "type": "url_citation", "text": chunk?.web?.title, "url": chunk?.web?.uri };
            }) ?? [];
    ...
    function onRequestFinished(message) { return parseBuffer(message); }
    function reset() { buffer = ""; }

    function buildScriptFileSetup(filePath) {
        const trimmedFilePath = trimFileProtocol(filePath);
        let content = ""
        content += `IMAGE_PATH='${shellSingleQuoteEscape(trimmedFilePath)}'\n`;
