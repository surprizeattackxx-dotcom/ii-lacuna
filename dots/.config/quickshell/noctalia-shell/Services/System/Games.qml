...
// Game library service (ported from illogical-impulse). Scans Steam/Heroic/ROMs
// via scan_games.py, manages favorites/hidden/launch-opts, launches games.
Singleton {
    id: root

    readonly property string stateDir: Quickshell.env("HOME") + "/.local/state/quickshell/games"
    readonly property string scanScript: Quickshell.shellPath("Scripts/games/scan_games.py")
    readonly property string installScript: Quickshell.shellPath("Scripts/games/install_heroic.py")
    readonly property string installStatePath: stateDir + "/install-state.json"
    ...
    // Ensure the state dir exists before any FileView writes.
    Process { id: dirInit; running: true; command: ["mkdir", "-p", root.stateDir] }

    FileView {
        id: favoritesFile
        path: Qt.resolvedUrl(root.stateDir + "/favorites.json")
        onLoaded: {
    ...
    FileView {
        id: hiddenFile
        path: Qt.resolvedUrl(root.stateDir + "/hidden.json")
        onLoaded: {
    ...
    FileView {
        id: launchOptsFile
        path: Qt.resolvedUrl(root.stateDir + "/launch-opts.json")
        onLoaded: {
    ...
    FileView {
        id: firstSeenFile
        path: Qt.resolvedUrl(root.stateDir + "/first-seen.json")
        onLoaded: {
