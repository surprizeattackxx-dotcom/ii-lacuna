...
BaseBubble {
    ...
    readonly property bool shouldShow: (island.timerRunning || island.timerRemainingSec > 0)
    ...
    // Pulsing opacity for the border when running
    property real _pulse: 1.0
    SequentialAnimation on _pulse {
        running: island.timerRunning
        loops: Animation.Infinite
        NumberAnimation { to: 0.25; duration: 700; easing.type: Easing.InOutSine }
        NumberAnimation { to: 1.0;  duration: 700; easing.type: Easing.InOutSine }
    }
    onIslandChanged: if (!island.timerRunning) _pulse = 1.0
    ...
    Rectangle {
        ...
        border.width: island.timerRunning ? 1.5 : 1
        border.color: island.timerRunning
            ? Qt.rgba(island.mauve.r, island.mauve.g, island.mauve.b, root._pulse * 0.85)
            : (island.glassTheme
                ? Qt.rgba(1, 1, 1, 0.18)
    ...
    Row {
        ...
        Text {
            ...
            color: island.timerRunning ? island.mauve : island.subtext0
        ...
        Text {
            ...
            color: island.timerRunning ? island.text : island.subtext0
            anchors.verticalCenter: parent.verticalCenter
            Behavior on color { ColorAnimation { duration: 200 } }
        }
    }
