...
PanelWindow {
    ...
    Process {
        id: musicProc
        command: ["bash", "-c", `~/.config/quickshell/ii/modules/islands/music/music_info.sh "${islandWindow.selectedPlayer}"`]
        stdout: StdioCollector {
            onStreamFinished: {
                try {
    ...
    Process {
        id: eqProc
        command: ["bash", "-c", "~/.config/quickshell/ii/modules/islands/music/equalizer.sh get"]
        stdout: StdioCollector {
            onStreamFinished: {
                try {
    ...
    Process {
        id: cavaProc
        command: ["bash", "-c", "cava -p ~/.config/quickshell/ii/modules/islands/music/cava_island.cfg 2>/dev/null"]
        running: false
        stdout: SplitParser {
            splitMarker: "\n"
            onRead: (line) => {
    ...
    Process {
        id: weatherProc
        command: ["bash", "-c",
            "data=$(~/.config/quickshell/ii/modules/islands/calendar/weather.sh --json 2>/dev/null); " +
            "ct=$(date +%H:%M); " +
                ...
                try {
                    ...
                    if (wsDotModel.count !== data.length) {
                        ...
                        for (let i = 0; i < data.length; i++)
                            wsDotModel.append({ "wsId": data[i].id.toString(), "wsState": data[i].state, "wsMonitor": data[i].monitor || "" })
                    } else {
                        for (let i = 0; i < data.length; i++) {
                            ...
                            if (wsDotModel.get(i).wsMonitor !== (data[i].monitor || ""))
                                wsDotModel.setProperty(i, "wsMonitor", data[i].monitor || "")
                        }
                    }
    ...
    Process {
        id: bgLumReader
        command: ["bash", "-c",
            "OUT=$(bash $HOME/.config/quickshell/ii/modules/islands/watchers/bg_luminance.sh " +
        ...
        Row {
            ...
            Repeater {
                ...
                delegate: Item {
                    ...
                    // Per-monitor blocks: island only shows its own screen's workspaces
                    property bool onThisMonitor:   !model.wsMonitor || model.wsMonitor === (islandWindow.screen?.name ?? "")
                    ...
                    width: (!onThisMonitor || isTrailingEmpty) ? 0 : s(16)
                    ...
                    MouseArea {
                        ...
                        hoverEnabled: true
                        cursorShape: Qt.PointingHandCursor
                        onClicked: Hyprland.dispatch("hl.dsp.focus({ workspace = " + parseInt(dotSlot.model.wsId) + " })")
                    }
                }
            }
        }
