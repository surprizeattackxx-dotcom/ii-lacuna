    ...
    Row {
        ...
        Repeater {
            ...
            delegate: Rectangle {
                ...
                // Per-monitor workspace blocks: only render this bar's monitor
                property bool onThisMonitor: !model.wsMonitor || model.wsMonitor === root.bar.screenName
                property bool isActive:   stateLabel === "active"
                property bool isOccupied: stateLabel === "occupied" || isActive

                visible: onThisMonitor
                width:  onThisMonitor ? bar.s(34) : 0
                height: bar.s(24)
                radius: width / 2
                ...
                MouseArea {
                    ...
                    onClicked: Hyprland.dispatch("hl.dsp.focus({ workspace = " + parseInt(wsName) + " })")
                }
            }
        }
    }
