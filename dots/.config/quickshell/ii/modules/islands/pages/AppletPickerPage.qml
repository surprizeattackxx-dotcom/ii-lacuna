...
Item {
    ...
    function setTheme(id) {
        Quickshell.execDetached(["bash", "-c",
            "jq --arg t \"$1\" '.topbarTheme = $t' ~/.config/quickshell/ii/modules/islands/settings.json " +
            "> /tmp/qs_settings_tmp.json && mv /tmp/qs_settings_tmp.json ~/.config/quickshell/ii/modules/islands/settings.json",
            "qs_theme", id
        ])
    }
