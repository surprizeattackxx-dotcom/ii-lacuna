...
// Reads ~/.config/quickshell/ii/modules/islands/settings.json once at startup and re-reads on
...
QtObject {
    ...
    property Process _settingsWatcher: Process {
        ...
        running: true
        command: ["bash", "-c",
            "while [ ! -f ~/.config/quickshell/ii/modules/islands/settings.json ]; do sleep 1; done; " +
            "inotifywait -qq -e modify,close_write,move_self,attrib ~/.config/quickshell/ii/modules/islands/settings.json"]
        stdout: StdioCollector {
            onStreamFinished: {
    ...
    property Process _colorsReader: Process {
        id: colorsReader
        running: false
        command: ["bash", "-c", "cat ~/.config/quickshell/ii/modules/islands/qs_colors.json 2>/dev/null || echo ''"]
        stdout: StdioCollector {
            onStreamFinished: {
    ...
    property Process _colorsWatcher: Process {
        ...
        running: false
        command: ["bash", "-c",
            "F=~/.config/quickshell/ii/modules/islands/qs_colors.json; " +
