...
if [[ -n "$focused_monitor" && -f "${Wallpapers.monitorStateDir}/$focused_monitor.json" ]]; then
    jq -r '.matugenPath // .thumbnailPath // .previewPath // .path // empty' "${Wallpapers.monitorStateDir}/$focused_monitor.json" 2>/dev/null
else
    first_state="$(find "${Wallpapers.monitorStateDir}" -name "*.json" 2>/dev/null | sort | head -1)"
    [[ -n "$first_state" ]] && jq -r '.matugenPath // .thumbnailPath // .previewPath // .path // empty' "$first_state" 2>/dev/null
fi
`]
