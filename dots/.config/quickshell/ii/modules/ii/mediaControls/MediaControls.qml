...
Scope {
    ...
    readonly property var realPlayers: {
        var p = MprisController.players;
        // Fall back to activePlayer if the filtered list is empty but a player
        // is actually active. Prevents the popup showing "No active player"
        // when isRealPlayer filtering is overly aggressive or not yet settled.
        if (p.length === 0 && MprisController.activePlayer) return [MprisController.activePlayer];
        return p;
    }
    readonly property var meaningfulPlayers: { var p = root.realPlayers; return root.filterDuplicatePlayers(p); }
    ...
    function filterDuplicatePlayers(players) {
        ...
        for (let i = 0; i < players.length; ++i) {
            ...
            for (let j = i + 1; j < players.length; ++j) {
                let p2 = players[j];
                if ((p1.trackTitle && p2.trackTitle && (p1.trackTitle.includes(p2.trackTitle) || p2.trackTitle.includes(p1.trackTitle))) || (p1.length > 0 && p2.length > 0 && Math.abs(p1.position - p2.position) <= 2 && Math.abs(p1.length - p2.length) <= 2)) {
                    group.push(j);
                }
            }
