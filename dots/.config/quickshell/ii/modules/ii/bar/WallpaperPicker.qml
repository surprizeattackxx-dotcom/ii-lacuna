    ...
    function applyWallpaper(safeFileName, isVideo) {
            ...
            if (alreadyExists) {
                const applyScript = `
                    (
                        ...
                        # DETERMINISTIC LOOP
                        for i in {1..20}; do
                            if bash "${switchScript}" --image "$DEST_FILE" --matugen-source "$FINAL_THUMB" --monitor "$(hyprctl monitors -j | jq -r ".[] | select(.focused) | .name")" >/dev/null 2>&1; then
                                break
                            fi
                            sleep 0.05
                        done
        ...
        if (isVideo) {
            ...
            wallpaperCmd = `
                ${ensureDaemonCmd}
                for i in {1..20}; do
                    if bash "${switchScript}" --image "$WALL_FILE" --matugen-source "$THUMB_FILE" --monitor "$(hyprctl monitors -j | jq -r ".[] | select(.focused) | .name")" >/dev/null 2>&1; then
                        break
                    fi
                    sleep 0.05
                done
