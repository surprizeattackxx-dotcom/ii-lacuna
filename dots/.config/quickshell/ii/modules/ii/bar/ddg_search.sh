...
# 1.5. Check for dependencies
if [[ ! -f "$SCRIPT_DIR/get_ddg_links.py" ]]; then
    echo "ERROR: Missing dependency $SCRIPT_DIR/get_ddg_links.py" >> "$LOG_FILE"
    echo "Please ensure get_ddg_links.py is in the same directory as this script."
    exit 1
fi
