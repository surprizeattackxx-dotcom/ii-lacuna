...
New file in `dots/.config/kitty/`, loaded via `watcher particles_watcher.py` in `kitty.conf`. Uses the real `on_focus_change`, `on_resize`, and `on_close` hooks:

- **On focus gained, and separately on resize**: starts a repeating `add_timer` callback (~50Hz, i.e. every 0.02s) scoped to that window, idempotently (a window that already has one running is left alone). Both hooks call the same `_ensure_timer` — **found live, post-ship**: kitty's `on_focus_change` only fires when its internal `is_focused` flag actually flips, which itself depends on an explicit OS/compositor "keyboard entered this window" event arriving cleanly (`kitty/window.py`'s `focus_changed`, driven by `boss.py`'s `on_focus`). For a window that's created and typed into immediately, that first transition doesn't always get reported in time, so relying on `on_focus_change` alone silently missed the very first burst of typing in a brand-new window. `on_resize` is a guaranteed fallback: every new window forces one resize/layout pass at creation (`kitty/window.py` sets `needs_layout = True` in `__init__`), so it always fires at least once regardless of the focus-event timing.
- **Each tick**: calls `window.as_text(add_cursor=True)`, parses the trailing `\x1b[row;colH` sequence and the plain grid text, and diffs against the previous tick's snapshot:
  ...
  - **typing past the terminal's width, wrapping to the next row without a real newline** (`curr.row == prev.row + 1`, `curr.col == 0`, `prev.col == cols - 1`) → still a `char` event, targeting the cell the cursor just left. **Found live, post-ship**: an earlier revision treated any row change identically (ignored), on the reasoning that "line wraps" were a rare edge case not worth the complexity — turned out to be wrong in practice (a real user typing a normal continuous line hits this constantly), so `Snapshot` now carries the screen's column count (`parse_snapshot(text, window.screen.columns)`) specifically to distinguish this case from a genuine Enter/new-line.
  - anything else (bigger jumps, an actual new line via Enter, unrelated redraws, scrollback, tab-completion inserting multiple characters at once) → no event. This is a deliberate side effect, not just a limitation: it means pastes and completions don't spam a burst per inserted character, only genuine one-key-at-a-time typing does.
